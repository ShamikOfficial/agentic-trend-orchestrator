...
Pipeline:
  ...
  7. FAISS-backed local store for embeddings + metadata
"""
...
from faiss_store import add_captions
...
def has_audio_stream(video_path: str) -> bool:
    ...
    if result.returncode != 0:
        raise RuntimeError(f"ffprobe error (audio stream probe):\n{result.stderr}")
...
def _clamp_seek_sec(ts: float, duration: float) -> float:
    """Keep seek times inside the decodable range (avoid EOF / no-frame seeks)."""
    if duration <= 0:
        return 0.0
    margin = min(0.1, duration / 2.0)
    return float(max(0.0, min(ts, duration - margin)))


def extract_frame(video_path: str, timestamp: float, out_image: str, *, duration: float | None = None) -> None:
    """
    Extract a single frame at `timestamp` seconds from the video.

    Uses PNG output to avoid MJPEG failing on limited-range YUV from codecs like AV1.
    """
    ts = _clamp_seek_sec(timestamp, duration) if duration is not None else max(0.0, timestamp)
    out = Path(out_image).with_suffix(".png")
    _run([
        "ffmpeg", "-y",
        "-ss", str(ts),
        "-i", video_path,
...
def caption_frame(image_path: str, prompt: str, transcript_hint: str = "") -> str:
    """Caption an image frame (PNG or JPEG) with GPT-4o vision."""
...
def ingest_video(
    ...
    """
    Ingest a video file into local FAISS store.
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if has_audio_stream(video_path):
            ...
            try:
                ...
                print(f"[ingest] Audio extraction failed ({e}), continuing without transcript")
        else:
            print("[ingest] No audio stream detected, continuing with visual-only ingest")
        ...
        for i in tqdm(range(n_chunks), desc="Chunks", unit="chunk"):
            ...
            # Extract frame at chunk midpoint (PNG: avoids MJPEG/YUV issues with AV1 etc.)
            frame_path = os.path.join(tmpdir, f"frame_{i:04d}.png")
            extract_frame(video_path, mid, frame_path, duration=duration)
        ...
        for i in range(n_chunks):
            caption_rows.append(
                {
                    "file_id": records["file_id"][i],
                    "file_path": records["file_path"][i],
                    "chunk_index": records["chunk_index"][i],
                    "start_sec": records["start_sec"][i],
                    "end_sec": records["end_sec"][i],
                    "caption": records["caption"][i],
                    "transcript": records["transcript"][i],
                    "embedding": records["embedding"][i],
                }
            )
        add_captions(caption_rows)
