"""
downloader.py — Download YouTube videos via yt-dlp and ingest them into FAISS-backed storage.
...
def download_url(url: str) -> tuple[str, str, float | None] | None:
    ...
    if ytdlp_bin:
        ...
        # PATH may not include scripts on Windows; try module invocation fallback.
    ...
    try:
        ...
        logger.error("yt-dlp not available (binary or python module).")
        print(
            "[downloader] yt-dlp not available. Install with "
            "`python -m pip install yt-dlp` in this same environment."
        )
        return None
...
def download_and_ingest(videos, platform: str = "youtube") -> list[str]:
    ...
    # Download all videos before ingestion.
    for item in items:
        url = item["url"]
